...
import HostelImage from '../assets/images/Hostel_Image.svg'
...
const ReviewCard = (props) => {
  ...
  return (
    <div
      className='w-full auto bg-white flex mt-10 mb-10 border-0 rounded-tl-lg rounded-bl-lg rounded-tr-lg rounded-br-lg'>
      {/* Details */}
      <div className='w-3/5 flex-col p-3 mb-5'>
        ...
        <div className='flex w-4/5 justify-between mt-5'>
          <div>
            <button
              className={`w-36 h-10 border-1 ${formData.status === 'del' ? 'bg-slate-400' : 'bg-defaultBg'}  border-slate-400 rounded-md`}
      ...
      <div className='w-2/5 mt-10 flex justify-end pr-5 mr-'>
        <div className='w-full flex-col'>
          <div className='flex justify-end'>
            <button className={`w-3/5 h-10 ${setColor()} flex justify-center pt-1 rounded-md pl-2 pr-2`}
                    disabled={true}>
              <p className='text-xl font-bold'>{display()}</p>
            </button>
          </div>
          <div className='flex justify-end'>
            <img src={HostelImage} className='w-3/5 h-auto mt-9' alt=''/>
          </div>
        </div>
      </div>
    </div>
