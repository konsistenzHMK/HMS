...
import ExpenseImage from '../assets/images/expenseImage.svg'
...
const Page1 = ({currentPage, formData, setFormData, nextPage, setCurrentPage}) => {
  ...
  const changeDateExpense = date => {
    ...
    setFormData((prevData) => ({
      ...prevData, doe: newDate
    }));
  }
  const changeDateBooking = date => {
    const newDate = date.target.value;
    setFormData((prevData) => ({
      ...prevData, dob: newDate
    }));
  }
                ...
                <div className='w-full h-auto flex-col mt-4'>
                  <div className=''>
                    <p className='mb-1 font-popins text-medium '>Expense Code <p
                      className='inline text-xl text-red-600'>*</p></p>
                  </div>
                  <div className='mt-1 '>
                    <input
                      type="text"
...
const Page2 = ({currentPage, formData, setFormData, nextPage, setCurrentPage}) => {
  ...
  const handleOptionSelect = (option) => {
    ...
    setFormData((prevData) => ({
      ...prevData, selection_option: option
    }));
  };
  return (<div className="flex bg-defaultBg">
      {/* Main Content */}
              ...
              <div className='w-7/12 flex-col pl-5 mb-10'>
                ...
                <form>
                  <div className='w-full h-auto flex-col mt-6'>
                    ...
                    <div className='mt-5'>
                      <div
                        className={`w-11/12 bg-gray-300 text-lg flex justify-center rounded-xl font-popins ${selectedOption === 'All' ? 'bg-gray-500 text-white' : ''} p-4 cursor-pointer`}
                        onClick={() => handleOptionSelect('All')}
                      >
                        <p className='font-popins'>All Students</p>
                      </div>
                      <div
                        className={`w-11/12 mt-3 text-lg bg-gray-300 flex justify-center rounded-xl font-popins ${selectedOption === 'Selected' ? 'bg-gray-500 text-white' : ''} p-4 cursor-pointer`}
...
const Page3 = ({currentPage, formData, setFormData, nextPage, setCurrentPage, previousPage}) => {
  ...
  const [selectedValues, setSelectedValues] = useState([{id: 1, name: 'John', hostel: 'Shruti Mam Academy'},
    {
      id: 2,
      name: 'Jane',
      hostel: 'Shruti Mam Academy'
    },
  ...
  const tableData = [{id: 1, name: 'John', hostel: 'Shruti Mam Academy'},
    {
      id: 2,
      name: 'Jane',
      hostel: 'Shruti Mam Academy'
  ...
  return (<div className="flex bg-defaultBg">
      ...
      <div className="w-full h-full">
        <form>
          ...
          <div className='w-full h-full bg-defaultBg flex justify-center font-popins'>
            <div
              className="bg-white w-11/12 h-auto mt-5 border-none rounded-lg flex justify-center font-popins mb-12 flex-col">
              ...
              <div className='w-full flex mb-10 '>
                ...
                <div className='w-1/2 flex justify-end align-middle'>
                  <button className='w-2/5 h-10 bg-gray-200 rounded-xl mt-5 mr-20 border-1 border-gray-400'
              ...
              <div className='w-full flex-col bg-blue-100 h-28'>
                ...
                <div className='flex justify-center font-popins w-full mb-5'>
                  <button className='w-1/4 h-10 bg-gray-300 rounded-xl mt-5 mr-5 border-1 border-gray-400'
                          onClick={Prevent}>Download for Excel
                  </button>
                  <button className='w-1/4 h-10 bg-gray-300 rounded-xl mt-5 ml-5 border-1 border-gray-400'
                          onClick={Prevent}>Upload from Excel
                  </button>
                </div>
              </div>
              ...
              <div className='w-full flex justify-center mb-10'>
                <div className='w-11/12 bg-grey2 mt-3 border-none rounded-lg'>
                  <table className='w-full'>
                    <thead>
                    <tr className='w-full'>
                      <th className='w-1/3 text-left text-orange-500 ml-5 font-popins text-lg h-12 '><p
                        className='ml-20'>Name</p></th>
                      <th className='w-1/3 text-center text-orange-500 font-popins text-lg'><p className=''>Student
                        Hostel Name </p></th>
                      <th className='w-1/3 text-center text-orange-500 font-popins text-lg'><p className=''>Select
                        Student </p></th>
                    </tr>
                    </thead>
                    <tbody className='w-full '>
                    {tableData.map((row) => (<tr key={row.id}
                                                 className={`step ${row.id % 2 ? 'w-full bg-gray-200 justify-center' : 'w-full bg-gray-100'}`}>
                      <td
...
const Page4 = ({currentPage, formData, setFormData, nextPage, setCurrentPage}) => {
  ...
  const handleSubmit = (e) => {
    ...
    if (Object.keys(formErrors).length === 0) {
      ...
      axios.post(`${SecretKeys.API_URL}/expense/header`, formData).then((response) => {
        // alert
...
const ExpenseForm = () => {
  const [formData, setFormData] = useState({
    exp_code: '', exp_name: '', exp_type: '', doe: '', dob: '', total_amount: '', vn: '', va: '', total_student: '',
  });
  ...
  return (<>
    ...
    {currentPage === 2 && (
      <Page2 currentPage={currentPage} formData={formData} setFormData={setFormData} nextPage={nextPage}
             previousPage={previousPage} setCurrentPage={setCurrentPage}/>)}
    {currentPage === 3 && (
      <Page3 currentPage={currentPage} formData={formData} setFormData={setFormData} nextPage={nextPage}
             previousPage={previousPage} setCurrentPage={setCurrentPage}/>)}
    {currentPage === 4 && (
      <Page4 currentPage={currentPage} formData={formData} setFormData={setFormData} nextPage={nextPage}
             previousPage={previousPage} setCurrentPage={setCurrentPage}/>)}
  </>)
}
