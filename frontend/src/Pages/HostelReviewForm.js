...
const HostelReviewForm = () => {
  ...
  const getAllSavedForms = async () => {
    try {
      const response = await fetch(`${SecretKeys.API_URL}/hostel/registration/saved`, {
        method: 'GET',
        headers: {
    ...
    <div className="w-full bg-defaultBg top-0 rounded-lg">
      ...
      {!ShowForm && !edit2 && !display &&
        <div className='w-full flex justify-center'>
          <div className='w-11/12 rounded-lg'> {allForms.map((item, index) => (
            <ReviewCard data={item} setShowForm={setShowForm} setFormDataOut={setFormData1} setEdit2={setEdit2}
