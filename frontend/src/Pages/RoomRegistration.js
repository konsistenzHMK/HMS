...
const RoomRegistartion = () => {
  ...
  const wing_name_and_id_fetch = async () => {
    try {
      const response = await fetch(`http://localhost:7000/get_wing_id_where_status_active?tower_id=${formData.tower_id}`, {
        method: "GET",
        headers: { "Content-Type": "application/json" },
      });
      if (response.ok) {
  ...
  const tower_name_and_id_fetch = async () => {
    try {
      const response = await fetch(`http://localhost:7000/get_tower_id_where_status_active?hostel_id=${formData.hostel_id}`, {
        method: "GET",
        headers: { "Content-Type": "application/json" },
      });
      if (response.ok) {
