...
const StudentReviewForm = () => {
  ...
  const getAllSavedForms = async () => {
    try {
      const response = await fetch(`${SecretKeys.API_URL}/get_student_form`, {
        method: 'GET', headers: {
  ...
  return (<div className="w-full bg-defaultBg top-0 h-full">
    
    {!ShowForm && !edit2 && !display && <div className='w-full flex justify-center'>
      <div className='w-11/12 '> {allForms.map((item, index) => (
        <ReviewCardStudent data={item} setShowForm={setShowForm} setFormDataOut={setFormData1} setEdit2={setEdit2}
                           setDisplay={setDisplay}/>))}</div>
    </div>}
