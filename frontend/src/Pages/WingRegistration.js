...
const WingRegistartion = () => {
  ...
  const handleSubmit = (e) => {
    ...
    if (Object.keys(formErrors).length === 0) {
      ...
      axios.post(`${SecretKeys.API_URL}/hostel_tower_wing`, formData).then((response) => {
  ...
  const wing_name_and_id_fetch = async (towerId) => {
    try {
      const response = await fetch(`${SecretKeys.API_URL}/get_wing_id_where_status_active?tower_id=${towerId}`, {
        method: "GET",
